...
void setup() {
  ...
  pinMode(SETTINGS_RESET, INPUT_PULLUP);
  attachInterrupt(CLK, readEncoder, CHANGE);
  attachInterrupt(DT, readEncoder, CHANGE);
  //attachInterrupt(SETTINGS_RESET, readEncoder, CHANGE);
...
void loop()
{
  ...
  if ((lastInputDelta != encoderPos) && remainingFrames > 0) {
    lastInputDelta = encoderPos;
    u8g2.clearBuffer();
    u8g2.drawStr(0, 10, String(fps).c_str());
    u8g2.drawStr(0, 20, String(encoderPos).c_str());
    u8g2.sendBuffer();
    remainingFrames--;
  }
  frameCounter++;
