...
{
	uint8_t a = 0x00;					 // GPIO base address (set in assembly)
	uint8_t b = 0x00;					 // Bitmask (set to 0x80 in assembly)
	uint8_t c = numBytes;			 // Byte count
	uint8_t d = 0x00;					 // Current byte (loaded in assembly)
	uint8_t f = 0x00;					 // Interrupt state
	uint32_t cc1 = 0, cc2 = 0; // Clock cycle counters
	pin = (1 << pin);
...
{
	uint8_t a = 0x00;			// GPIO address placeholder (set in assembly)
	uint8_t b = 0x00;			// Bit mask (set in assembly to 0x80)
	uint8_t c = numBytes; // Byte counter
	uint8_t d = 0x00;			// Current byte (loaded from data array in assembly)
	uint8_t e = 0x00;			// Unused
	uint8_t f = 0x00;			// Interrupt state
	uint32_t cc1 = 0;			// Clock cycle counter 1
	uint32_t cc2 = 0;			// Clock cycle counter 2
	pin1 = (1 << pin1);
	pin2 = (1 << pin2);
...
{
	uint8_t a = 0x00;										// GPIO address (set in assembly)
	uint8_t b = 0x00;										// Bitmask (set in assembly to 0x80)
	uint8_t c = numBytes;								// Byte counter
	uint8_t d = 0x00;										// Current byte (loaded in assembly)
	uint8_t f = 0x00;										// Interrupt state
	uint32_t cc1 = 0, cc2 = 0, cc3 = 0; // Clock cycle counters
	pin = (1 << pin);
